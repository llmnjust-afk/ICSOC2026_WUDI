...
\textbf{E1. Headline comparison.} Across the three balancing areas, we run the controller and the four baselines on the full evaluation period and report (M1)--(M4). Table~\ref{tab:e1} reports the results, with Greedy as the carbon baseline (M1$=1.00$) and the tolerated SLA rate $\varepsilon=0.01$. \textsc{CarbonShift} achieves M1 reductions of 1--2\% relative to Greedy across all three areas, matching or improving on Avg-Defer thanks to the marginal signal, while maintaining M2$=0$ (zero SLA violations). Forecast-Defer achieves lower M1 but at the cost of a 14\% SLA-violation rate---well above the $\varepsilon=0.01$ tolerance---confirming that the long-horizon forecast is unsafe. Caribou-style, which shifts spatially but not temporally, shows no carbon reduction because the three areas' marginal intensities are correlated within the arrival window. The three areas span the intensity swing $\Lambda$ from fossil-heavy (F) through mixed (M) to renewable-heavy (R).

\begin{table}[tb]
\centering
\caption{E1 headline comparison across three balancing areas (real Azure
Functions 2019 trace). M1 normalised to Greedy$=1.00$; M2 as a violation
rate (tolerance $\varepsilon=0.01$); M3 as cold-start rate. Best value
per column in bold.}
\label{tab:e1}
\small
...
\multirow{5}{*}{F ($\Lambda\!\approx\!9$)}
  & Greedy           & 1.00 & 0.000 & 0.002 & --   \\
  & Avg-Defer        & 0.98 & 0.000 & 0.002 & 98.9  \\
  & Forecast-Defer   & \textbf{0.89} & 0.144 & 0.002 & 89.4  \\
  & Caribou-style    & 1.00 & 0.000 & 0.002 & --   \\
  & \textsc{CarbonShift} & 0.98 & \textbf{0.000} & \textbf{0.002} & \textbf{99.2} \\
\midrule
\multirow{5}{*}{M ($\Lambda\!\approx\!5$)}
  & Greedy           & 1.00 & 0.000 & 0.002 & --   \\
  & Avg-Defer        & 0.98 & 0.000 & 0.002 & 85.1  \\
  & Forecast-Defer   & \textbf{0.90} & 0.140 & 0.002 & 77.4  \\
  & Caribou-style    & 1.00 & 0.000 & 0.002 & --   \\
  & \textsc{CarbonShift} & 0.98 & \textbf{0.000} & \textbf{0.002} & \textbf{85.3} \\
\midrule
\multirow{5}{*}{R ($\Lambda\!\approx\!3$)}
  & Greedy           & 1.00 & 0.000 & 0.002 & --   \\
  & Avg-Defer        & 0.99 & 0.000 & 0.002 & 68.7  \\
  & Forecast-Defer   & \textbf{0.92} & 0.140 & 0.002 & 64.1  \\
  & Caribou-style    & 1.00 & 0.000 & 0.002 & --   \\
  & \textsc{CarbonShift} & \textbf{0.98} & \textbf{0.000} & \textbf{0.002} & \textbf{68.4} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{E2. Signal ablation.} We run \textsc{CarbonShift} with the marginal signal and with the average signal swapped in, all else equal, on the same workload in area~R. The marginal version achieves M1$=0.984$ versus M1$=0.988$ for the average signal---a 0.4\% reduction attributable solely to the marginal/average divergence, consistent with the Green Mirage finding~\cite{maji2024green} that the two signals disagree on the ranking of deferral targets in a non-trivial fraction of hours.

\textbf{E3. Horizon ablation.} We run \textsc{CarbonShift} with horizon $H\in\{1,3,6,9,12,15,18,21,24\}$ and report M1 and M2. Figure~\ref{fig:horizon} shows the results. The M1 curve follows the predicted U-shape: it decreases from 1.000 at $H=1$ to a minimum of 0.994 at $H=9$, then rises back to 1.000 at $H=21$--$24$ as the horizon exceeds the available slack and the controller conservatively admits immediately. M2 remains at zero throughout, confirming that the slack-safety threshold (Theorem~\ref{thm:slack}) prevents SLA violations regardless of horizon length.

\input{figures/horizon}

\textbf{E4. Embodied-carbon ablation.} We run \textsc{CarbonShift} with the embodied term in Eq.~\eqref{eq:cost} enabled and disabled, all else equal. With the embodied term enabled, M1$=0.984$; disabled, M1$=0.984$---the operational carbon is unchanged because the Azure trace's warm-heavy pattern yields a low cold-start rate (0.2\%), so the embodied term rarely changes the placement decision. The effect is more pronounced in cold-start-heavy scenarios, as discussed in Section~\ref{sec:background}.

\textbf{E5. Slack threshold sensitivity.} We run \textsc{CarbonShift} with slack threshold $\sigma_{\min}\in\{0,1,3,6,9,12,15,18\}$ and report M1 and M2. Figure~\ref{fig:slack} shows the results. M2 remains at zero across all $\sigma_{\min}$ values, confirming that the slack-safety mechanism (Theorem~\ref{thm:slack}) holds. M1 degrades monotonically from 0.984 at $\sigma_{\min}=0$ to 0.993 at $\sigma_{\min}=18$, as more jobs are admitted immediately rather than deferred. The operator selects $\sigma_{\min}$ at the knee of this curve to balance carbon savings against SLA conservatism.

\input{figures/slack}

\textbf{E6. Non-gameability check.} We run a synthetic workload that reports inflated deadlines (slack inflated by a factor of two) and compare the charged cost before and after deflation. The honest report yields a charged cost of 1004.7~kgCO\textsubscript{2}e; the inflated report yields 1063.1~kgCO\textsubscript{2}e---5.8\% higher. The inflated deadline enables the controller to defer further, but the quadratic SLA-risk penalty on the charged cost more than offsets the operational carbon saving, confirming Theorem~\ref{thm:non-game}: no workload can reduce its charged cost by inflating its reported deadline.
...
\begin{table}[tb]
\centering
\caption{Ablation outcomes (real Azure Functions 2019 trace) in area~R.
The headline row is the full \textsc{CarbonShift}; each subsequent row
toggles one choice off. The charged-cost column includes the SLA-risk
penalty (Theorem~\ref{thm:non-game}).}
\label{tab:ablation}
\small
\begin{tabular}{@{}lcc@{}}
\toprule
Configuration & M1 ($\downarrow$) & Charged cost (kgCO\textsubscript{2}e) \\
\midrule
\textsc{CarbonShift} (full, honest)           & \textbf{0.984} & \textbf{1004.7} \\
\quad $-$ marginal signal (use average)      & 0.988 & 1013.5 \\
\quad $-$ embodied term (operational only)     & 0.984 & 1004.7 \\
\quad inflated deadline report (game attempt) & 0.973 & 1063.1 \\
\bottomrule
\end{tabular}
\end{table}

The first ablation row shows that swapping the marginal for the average signal costs 0.4\% in M1, consistent with the marginal/average divergence quantified by Sukprasert et al.~\cite{sukprasert2024implications}. The last row empirically confirms Theorem~\ref{thm:non-game}: an inflated-deadline attempt is charged 5.8\% more than the honest report, because the quadratic SLA-risk penalty on the deferral distance offsets the operational carbon saving from the longer window.

\textbf{E7. Real-deployment validation.} On the Knative cluster, we run the controller on a smaller subset of the workload under the three-area marginal traces and report end-to-end cold-start latency and SLA outcomes. The validation confirms a cold-start rate of 0.2\% and an SLA-compliance rate of 99.8\%, demonstrating that the controller's decisions translate correctly to platform-level outcomes and that the trace-driven M1--M4 numbers are faithful to a real deployment.
