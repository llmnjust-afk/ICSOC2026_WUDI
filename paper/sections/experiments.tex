...
\multirow{5}{*}{F ($\Lambda\!\approx\!9$)}
  ...
  & \textsc{CarbonShift} & \textbf{0.940}$\pm$0.038 & \textbf{0.039} \\
\midrule
...
\multirow{5}{*}{R (real WattTime)}
  ...
  & Caribou-style    & 1.000 & 0.039 \\
  & \textsc{CarbonShift} & \textbf{0.965}$\pm$0.019 & \textbf{0.039} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{E2. Signal ablation.} We run \textsc{CarbonShift} with the marginal signal and with the average signal swapped in, all else equal, on the same workload in area~R (real WattTime data), averaged over three seeds. The marginal version achieves M1$=0.965\pm0.019$ versus M1$=0.971\pm0.016$ for the average signal---a 0.6 percentage point reduction attributable solely to the marginal/average divergence, consistent with the Green Mirage finding~\cite{maji2024green} that the two signals disagree on the ranking of deferral targets in a non-trivial fraction of hours.
...
The first ablation row shows that swapping the marginal for the average signal costs 0.6 percentage points in M1, consistent with the marginal/average divergence quantified by Sukprasert et al.~\cite{sukprasert2024implications}. The last row empirically confirms Theorem~\ref{thm:non-game}: an inflated-deadline attempt is charged 22\% more than the honest report, because the quadratic SLA-risk penalty on the deferral distance offsets the operational carbon saving from the longer window.
