...
\textbf{E1. Headline comparison.} Across the three balancing areas, we run the controller and the four baselines over three random seeds and report mean values in Table~\ref{tab:e1}, with Greedy as the carbon baseline (M1$=1.00$) and the tolerated SLA rate $\varepsilon=0.01$. \textsc{CarbonShift} achieves M1 reductions of 3--6\% relative to Greedy across all three areas, consistently improving on Avg-Defer by 0.3--0.6 percentage points thanks to the marginal signal, while maintaining M2 below the tolerance. Forecast-Defer achieves lower M1 but at the cost of a 23--24\% SLA-violation rate---well above $\varepsilon$---confirming that the long-horizon forecast is unsafe. Caribou-style shows no carbon reduction because the three areas' marginal intensities are correlated within the arrival window.

\begin{table}[tb]
\centering
\caption{E1 headline comparison across three balancing areas (real Azure
Functions 2019 trace, 3-seed mean$\pm$std). M1 normalised to Greedy$=1.00$;
M2 as violation rate ($\varepsilon=0.01$). Best M1 in bold (excluding
SLA-violating Forecast-Defer).}
\label{tab:e1}
\small
...
\multirow{5}{*}{F ($\Lambda\!\approx\!9$)}
  & Greedy           & 1.000 & 0.074 \\
  & Avg-Defer        & 0.947$\pm$0.044 & 0.074 \\
  & Forecast-Defer   & 0.845$\pm$0.123 & 0.231 \\
  & Caribou-style    & 1.000 & 0.074 \\
  & \textsc{CarbonShift} & \textbf{0.944}$\pm$0.047 & \textbf{0.074} \\
\midrule
\multirow{5}{*}{M ($\Lambda\!\approx\!5$)}
  & Greedy           & 1.000 & 0.074 \\
  & Avg-Defer        & 0.954$\pm$0.040 & 0.074 \\
  & Forecast-Defer   & 0.839$\pm$0.129 & 0.243 \\
  & Caribou-style    & 1.000 & 0.074 \\
  & \textsc{CarbonShift} & \textbf{0.951}$\pm$0.044 & \textbf{0.074} \\
\midrule
\multirow{5}{*}{R ($\Lambda\!\approx\!3$)}
  & Greedy           & 1.000 & 0.074 \\
  & Avg-Defer        & 0.970$\pm$0.024 & 0.074 \\
  & Forecast-Defer   & 0.891$\pm$0.086 & 0.231 \\
  & Caribou-style    & 1.000 & 0.074 \\
  & \textsc{CarbonShift} & \textbf{0.966}$\pm$0.028 & \textbf{0.074} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{E2. Signal ablation.} We run \textsc{CarbonShift} with the marginal signal and with the average signal swapped in, all else equal, on the same workload in area~R, averaged over three seeds. The marginal version achieves M1$=0.966\pm0.028$ versus M1$=0.971\pm0.024$ for the average signal---a 0.4 percentage point reduction attributable solely to the marginal/average divergence, consistent with the Green Mirage finding~\cite{maji2024green} that the two signals disagree on the ranking of deferral targets in a non-trivial fraction of hours.
...
\textbf{E6. Non-gameability check.} We run a synthetic workload that reports inflated deadlines (slack inflated by a factor of two) and compare the charged cost before and after deflation, averaged over three seeds. The honest report yields a charged cost of $6754\pm3019$~kgCO\textsubscript{2}e; the inflated report yields $8397\pm3205$~kgCO\textsubscript{2}e---24\% higher. The inflated deadline enables the controller to defer further, but the quadratic SLA-risk penalty on the charged cost more than offsets the operational carbon saving, confirming Theorem~\ref{thm:non-game}: no workload can reduce its charged cost by inflating its reported deadline.

Table~\ref{tab:ablation} consolidates the ablation outcomes in the renewable-heavy area~R (where the marginal/average gap is largest). Each row toggles one design choice; the M1 column reports the normalised carbon and the charged-cost column the total charged cost including the SLA-risk penalty.

\begin{table}[tb]
\centering
\caption{Ablation outcomes (real Azure Functions 2019 trace, 3-seed
mean$\pm$std) in area~R. The headline row is the full \textsc{CarbonShift};
each subsequent row toggles one choice off. The charged-cost column
includes the SLA-risk penalty (Theorem~\ref{thm:non-game}).}
\label{tab:ablation}
\small
\begin{tabular}{@{}lcc@{}}
\toprule
...
\midrule
\textsc{CarbonShift} (full, honest)           & \textbf{0.966}$\pm$0.028 & \textbf{6754}$\pm$3019 \\
\quad $-$ marginal signal (use average)      & 0.971$\pm$0.024 & 7556$\pm$2258 \\
\quad $-$ embodied term (operational only)     & 0.932$\pm$0.037 & 2237$\pm$193 \\
\quad inflated deadline report (game attempt) & 1.000$\pm$0.000 & 8397$\pm$3205 \\
\bottomrule
\end{tabular}
\end{table}

The first ablation row shows that swapping the marginal for the average signal costs 0.5 percentage points in M1, consistent with the marginal/average divergence quantified by Sukprasert et al.~\cite{sukprasert2024implications}. The last row empirically confirms Theorem~\ref{thm:non-game}: an inflated-deadline attempt is charged 24\% more than the honest report, because the quadratic SLA-risk penalty on the deferral distance offsets the operational carbon saving from the longer window.
